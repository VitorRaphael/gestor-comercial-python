...
- **Sub-modelo** (`produtos.subcategoria`, opcional): subdivisão de catálogo **dentro** da categoria — "Lanches" → "Artesanal", "Podrão", "Cachorro Quente". É texto livre, não uma entidade: não tem impressora, status nem regra. Serve para organizar o cadastro (badge e filtro na tela de Cardápio) e para diferenciar variações na hora de lançar (metadado e busca no modal "Adicionar item"). **Não participa do roteamento de impressão** — ver §3.12. Grafia divergente na mesma categoria ("podrao" onde já existe "Podrão") é canonizada pelo service para a grafia existente, senão o texto livre viraria dois grupos com o mesmo nome.
...
- **Roteamento da comanda de produção** (porte de `RoteamentoImpressaoService.java`): os itens são agrupados por `item.produto.categoria.impressora`, preservando a ordem de lançamento — a cozinha lê o cupom na sequência em que o atendente digitou. Sai um cupom por impressora.
  - **A categoria é o ÚNICO eixo de roteamento.** O sub-modelo do produto (§3.2) é organização de catálogo e não abre fila: dois itens da mesma categoria com sub-modelos diferentes saem no mesmo cupom, e o sub-modelo não é impresso. `tests/unit/test_impressao_service.py` prova o agrupamento e ainda varre o código-fonte do `impressao_service` para reprovar a menção à `subcategoria`.
