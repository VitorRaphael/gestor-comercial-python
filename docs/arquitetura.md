...
- **Subcategoria** (entidade `Subcategoria`, `produtos.subcategoria_id` opcional): subdivisão de catálogo **dentro** da categoria — "Lanches" → "Artesanal", "Podrão", "Cachorro Quente". Tem CRUD próprio (criar, renomear, excluir) porque precisa poder nascer **vazia**, esperando os itens: o gerente planeja a organização antes de classificar. Nome único dentro da categoria, com a comparação ignorando acento e caixa; o mesmo nome em categorias diferentes são duas subdivisões independentes. Não tem `ativo` — desativar não significaria nada que excluir já não signifique, e excluir é seguro porque os produtos apenas voltam a ficar sem subcategoria (`ondelete="SET NULL"`). Excluir a **categoria**, essa sim, leva as subdivisões dela junto. **Não participa do roteamento de impressão** — ver §3.12.
...
- 2026-09-06 — **Remasterização V1, Fase 2: núcleo de dados e performance.** Plano completo em [`REMASTERIZACAO-V1.md`](../REMASTERIZACAO-V1.md); o que muda na arquitetura:
  - **Índices nas 20 chaves estrangeiras** (`index=True` no `domain/` + migration `b4d7e2c91a08`). O SQLite nunca cria índice para FK sozinho, então toda consulta quente era varredura de tabela inteira — o custo que crescia com as semanas de operação.
  - **`journal_mode = WAL`** (decisão do Vitor). O motivo principal não é velocidade e sim **concorrência**: em WAL leitura e escrita deixam de se bloquear, e é isso que o App Mobile do Atendente (backlog §5) vai exigir. Escolher agora evita migrar o banco em produção depois. **`synchronous` fica em `FULL`** — baixar para `NORMAL`, como todo guia de WAL sugere, protegeria contra o app morrer mas **não** contra queda de energia no meio do commit, que é o cenário do food truck (ver `test_resiliencia_queda_energia.py`, e a correção da nota de 2026-08-22 abaixo, escrita quando o `journal_mode` ainda era o padrão).
  - **`repository/backup.py`** (funcionalidade nova, exceção deliberada à regra "só faxina" da remasterização — é ela que torna o WAL seguro): backup por **`VACUUM INTO`**, que grava um `.db` único e já consolidado, em vez de cópia de arquivo — que com WAL deixaria as últimas vendas no `-wal` para trás. Roda automático a cada fechamento de caixa e sob demanda em **Configurações → Cópia de Segurança**; `wal_checkpoint(TRUNCATE)` no fechamento de caixa e no encerramento do app mantêm o `.db` completo no disco.
...
- 2026-09-08 — **A fileira de marcadores do teclado de PIN passou a ter o tamanho da senha cadastrada.** Defeito relatado pelo Vitor: o modal do Caixa mostrava seis bolinhas para a Senha Operacional, que de fábrica tem oito. O piso era uma constante e não podia deixar de ser — SHA-256 é via de mão única e do hash não sai nem o valor nem o comprimento —, então `loja_config` ganhou `senha_master_tamanho`/`senha_operacional_tamanho`/`senha_login_tamanho` (migração `c1d5b8e37a42`), gravados no bootstrap e em cada `alterar_senha_*`; `LojaConfigService.tamanho_da_senha(nivel)` e `AuthService.tamanho_do_pin_nivel(nivel)` expõem só o comprimento, nunca o valor. `PinPadDialog` recebe o número como `tamanho_esperado` e o usa de piso da fileira, que continua elástica para cima até o teto de 12 (limite de largura do cartão, com teste que refaz a conta). Como o diálogo é construído a cada acesso, trocar a senha em Senhas e Acesso já muda a contagem no acesso seguinte, sem sinal nem recarga. O ENTRAR **não** foi amarrado ao enchimento da fileira, e há teste para isso: o Nível 2 aceita a Operacional (8) ou a Master (6, cascata do §3.13), então exigir a fileira cheia faria a Master parar de abrir o Caixa. O backfill da migração confere o hash do padrão de fábrica com o salt gravado em vez de chutar: bateu, grava o tamanho; não bateu, a senha já foi trocada, o comprimento é desconhecido e a coluna fica `NULL` (a tela cai no piso de 6 até a próxima troca). O que isto entrega a quem tenha o arquivo do banco é saber que a senha tem N dígitos — irrelevante contra 4 a 8 dígitos numéricos, que um ataque offline percorre inteiro em segundos de qualquer forma, e a própria tela mostra a contagem a quem está de pé na frente do monitor. Junto veio a correção de um problema latente achado ao tentar testar a migração: `migrations/env.py` sobrescrevia `sqlalchemy.url` com o `DB_PATH` resolvido no import de `repository/base.py`, então `command.upgrade(config, ...)` dentro de um teste migrava o **banco real da máquina de quem roda a suíte** — nem `monkeypatch` da variável de ambiente nem `set_main_option` no teste tinham efeito. Agora o `env.py` só impõe o `DB_PATH` quando o chamador não escolheu banco (placeholder do `alembic.ini`), e a migração tem teste de verdade, em arquivo temporário, com upgrade/downgrade/upgrade. Suíte em **1025 testes** (de 1009), 100% verde.
- 2026-09-09 — **Sub-modelo do produto (§3.2).** Pedido do Vitor: o cardápio real tem 15 categorias e 113 produtos, e dentro de "Lanches" convivem artesanal, podrão e cachorro quente. A categoria não podia ser quebrada em três porque é ela que decide a impressora, então o produto ganhou uma segunda etiqueta, puramente organizacional, dentro dela — `produtos.subcategoria` (`TEXT NULL`, migração `e2c7b4f9a613`) mais o índice composto `idx_produtos_categoria_sub (categoria_id, subcategoria)`, que o `EXPLAIN QUERY PLAN` confirma ser *covering*: a consulta que monta as sugestões e as pílulas responde sem abrir uma linha de produto. **A regra de ouro é que o roteamento de impressão continua 100% amarrado à categoria**, e ela está travada em três alturas: um teste que manda três lanches da mesma categoria com sub-modelos diferentes saírem num cupom só, uma varredura que reprova a palavra `subcategoria` dentro do `impressao_service` (mesmo mecanismo que o §9.5 usou para o ciano emprestado) e um teste de migração que confere `categorias.impressora_id` intacto; fora da suíte, `tools/comparar_cupons.py` deu os cupons idênticos linha a linha. Não virou tabela com FK de propósito — sub-modelo não tem impressora, status nem regra, e uma FK custaria CRUD, tela e tratamento de órfão para guardar só um nome. O preço de ser texto livre é a divergência de grafia, e quem paga é o service: `_subcategoria_canonica` compara ignorando acento, caixa e espaço repetido e grava **a grafia que a categoria já usava**, porque pílula de sugestão sozinha só cobre quem clica em vez de digitar. A normalização saiu para `services/texto.py` porque a busca precisa da mesma regra — duas cópias divergindo fariam o mesmo sub-modelo agrupar numa tela e separar na outra. Nas telas, tudo some quando não há sub-modelo cadastrado, e é por isso que as 24 renderizações da bancada continuam idênticas byte a byte: a funcionalidade é invisível até o cardápio ser classificado à mão (o `seed.py` não classifica nada — ele pula produto que já existe pelo nome, e classificar lá deixaria a máquina do food truck e uma máquina limpa com cardápios diferentes). A renderização com dado real pegou um defeito que a suíte não pegava: com o sub-modelo "Cachorro Quente" o selo crescia até o `QLabel` do nome, que corta **sem reticências**, e "Cachorro Quente Linguiça" virava "Cachorro Quente Lin". Corrigir esbarrou em duas armadilhas do Qt, ambas registradas em `REMASTERIZACAO-V1.md` §9.8 — `QFontMetrics.elidedText` **ignora** o `letterSpacing`, e fonte de QSS vence `setFont` (a solução foi `ensurePolished()` e medir a fonte do próprio rótulo, sem gêmeo para divergir). De quebra, `_ProdutoDialog.resultado()` virou a dataclass `DadosProduto`: a tupla iria a sete posições desempacotadas por ordem, e `descricao`/`subcategoria` são ambas `str | None` — trocar as duas gravaria uma no lugar da outra, calado. Suíte em **1241 testes** (de 1179), 100% verde, conferidos com 18 mutações que reprovam.
- 2026-09-09 — **A subcategoria vira entidade, e a tela de Cardápio vira a hierarquia (§3.2).** O Vitor olhou a tela do item anterior e apontou que ela estava de cabeça para baixo: a subcategoria aparecia como um selo pendurado na linha do produto, quando é ela que **contém** os produtos. Junto veio o pedido de uma tela para criar subcategorias — e é essa segunda parte que mudou o modelo, não o desenho: com a coluna de texto, uma subcategoria só existia enquanto algum produto carregasse a string, então não havia como criar uma vazia esperando os itens, e renomear exigiria reescrever cada produto que a carregasse. Virou tabela própria (`subcategorias`, par categoria+nome único) com `produtos.subcategoria_id` no lugar do texto, migração `f8d1a6c40b27` convertendo o dado existente pelo par INTEIRO (casar só pelo nome faria um "Podrão" de Lanches apontar para o de Porções). A tela passou a ser uma árvore Categoria → Subcategoria à esquerda e uma tabela agrupada por subdivisão à direita; o modal "Nova subcategoria" é o sétimo modal em cartão do app. A regra de ouro do §3.12 continua trancada nas mesmas três alturas, e a bancada de cupons deu os dois idênticos linha a linha. Três defeitos de layout do Qt ficaram registrados em `REMASTERIZACAO-V1.md` §9.9, e vale lê-los antes de mexer em tabela ou árvore: `setItemWidget` **não** dimensiona o item (era o corte que o Vitor viu), `stretchLastSection` nasce ligado, e `setStyleSheet` num widget-pai desce para os filhos vencendo o QSS global. Suíte em **1284 testes** (de 1225), 100% verde, conferidos com 23 mutações que reprovam.
