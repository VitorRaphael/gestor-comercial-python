...
class ComandaRepository(Repository[Comanda]):
    ...
    def fechadas_do_caixa(self, caixa_id: int) -> int:
        """Quantas comandas o turno fechou.

        Uma consulta de agregação, sem trazer comanda nenhuma: os resumos
        financeiros só querem o número. Carregar as centenas de comandas do
        caixa como objetos ORM só para chamar `len()` gastava memória e tempo à
        toa numa máquina fraca — e no Dashboard Mensal isso acontece uma vez por
        turno do mês (§3.6).

        Entre o §9.23 e o §9.26 esta consulta devolvia também a soma da taxa de
        serviço do turno. A taxa foi removida do sistema, e com ela a segunda
        coluna do retorno.

        Só FECHADA: é a venda concluída. Comanda em conferência ainda pode ser
        reaberta, e o fechamento do caixa já recusa turno com conta em aberto.
        """
        stmt = select(func.count()).where(
            Comanda.caixa_id == caixa_id, Comanda.status == StatusComanda.FECHADA
        )
